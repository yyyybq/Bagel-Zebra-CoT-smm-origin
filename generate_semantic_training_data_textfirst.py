...
# 全局变量，用于存储图片路径模板（将在main函数中设置）
BLOCK_IMAGE_PATH_TEMPLATE = ""
BLOCK_IMAGE_PATH_TEMPLATE_NAME = ""
...
def get_block_image_path(block: Dict) -> str:
    """根据block的字段选择合适的图片路径"""
    if 'type' in block and 'color' in block:
        # 有type和color字段，使用原路径
        return BLOCK_IMAGE_PATH_TEMPLATE.format(block_type=block['type'], color=block['color'])
    elif 'name' in block:
        # 只有name字段，使用新路径
        return BLOCK_IMAGE_PATH_TEMPLATE_NAME.format(name=block['name'])
    else:
...
def get_unique_blocks(blocks: List[Dict]) -> tuple:
    """获取去重后的积木列表和原始索引到去重索引的映射
    
    Returns:
        (unique_blocks, index_mapping): 去重后的积木列表，以及原始索引到去重索引的映射
    """
    unique_blocks = []
    seen_blocks = {}  # 用于存储已见过的积木特征
    index_mapping = {}  # 原始索引 -> 去重后的索引
    
    for i, block in enumerate(blocks):
        # 生成积木的唯一标识符
        block_key = get_block_image_path(block)
        
        if block_key not in seen_blocks:
            # 新的积木类型
            seen_blocks[block_key] = len(unique_blocks)
            unique_blocks.append(block)
            index_mapping[i] = len(unique_blocks) - 1
        else:
            # 重复的积木
            index_mapping[i] = seen_blocks[block_key]
    
    return unique_blocks, index_mapping


def generate_question_template(shape_name: str, blocks: List[Dict], unique_blocks: List[Dict], category: str = "") -> str:
    """生成问题模板"""
    ...
    question += f"The initial input includes {total_unique_blocks} images of multiple blocks that will be used -- "

    for i in range(1, total_unique_blocks + 1):
        question += f"block {i}: <image_start>[problem_image_{i}]<image_end> "

    question += f"and an image of the final desired shape: <image_start>[problem_image_{total_unique_blocks+1}]<image_end>. "
    ...
    question += f" Step 0 has been completed: a {first_block_desc} block has been placed on top of the ground. The image after step 0 is provided: <image_start>[problem_image_{total_unique_blocks+2}]<image_end>. "
...
def generate_image_paths(scene_folder: Path, shape_name: str, blocks: List[Dict], unique_blocks: List[Dict], view_num: int = 0) -> Dict[str, str]:
    """生成所有图片路径的映射"""
    image_paths = {}
    
    # problem_image_1 到 problem_image_{m}: 每个去重后的积木的多视角图片 (m = len(unique_blocks))
    for i, block in enumerate(unique_blocks):
        block_image_path = get_block_image_path(block)
        image_paths[f"problem_image_{i+1}"] = block_image_path
    
    # problem_image_{m+1}: 最终状态图片 (使用最后一个step的图片)
...
def process_scene(scene_folder: Path, view_num: int = 0) -> Dict[str, Any]:
    ...
    # 获取去重后的积木列表
    unique_blocks, index_mapping = get_unique_blocks(blocks)
    
    # 生成各个部分
    question = generate_question_template(shape_name, blocks, unique_blocks)
    thought_trace = generate_thought_trace(shape_name, blocks, total_blocks)
    final_answer = generate_final_answer(total_blocks)
    image_paths = generate_image_paths(scene_folder, shape_name, blocks, unique_blocks, view_num)
    ...
    training_data = {
        ...
        "unique_blocks": unique_blocks,  # 添加去重后的blocks
        "block_index_mapping": index_mapping,  # 添加索引映射
    }
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='Generate semantic training data from block assembly scenes')
    parser.add_argument('--block-image-folder-type-color', 
                        default='/lustre/fsw/portfolios/nvr/users/ymingli/projects/ljh/semantic_all/semantic_all/each_block_views_diffpose',
                        help='Path to block images folder for type/color format (default: each_block_views_diffpose)')
    parser.add_argument('--block-image-folder-name',
                        default='/lustre/fsw/portfolios/nvr/users/ymingli/projects/ljh/semantic_all/semantic_all/semantic_block_views_diffposes2',
                        help='Path to block images folder for name format (default: semantic_block_views_diffposes2)')
    ...
    # 设置全局图片路径模板
    global BLOCK_IMAGE_PATH_TEMPLATE, BLOCK_IMAGE_PATH_TEMPLATE_NAME
    BLOCK_IMAGE_PATH_TEMPLATE = args.block_image_folder_type_color + "/{block_type}_{color}.png"
    BLOCK_IMAGE_PATH_TEMPLATE_NAME = args.block_image_folder_name + "/{name}.png"
    ...
    print(f"Block image folder (type/color): {args.block_image_folder_type_color}")
    print(f"Block image folder (name): {args.block_image_folder_name}")
    ...
    # 保存所有视角的JSONL文件
    all_views_file = output_folder / "semantic_training_all_views_textfirst.jsonl"
    ...
    # 保存每个单独视角的JSONL文件
    for view_num in view_nums:
        view_file = output_folder / f"semantic_training_view{view_num}_textfirst.jsonl"
...
# python generate_semantic_training_data_textfirst.py