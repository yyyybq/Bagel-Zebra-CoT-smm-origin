...
def main():
    # 设置输入输出路径
    input_folder = Path("/lustre/fsw/portfolios/nvr/users/ymingli/projects/ljh/semantic_blocks_part1/")
    output_file = Path("./semantic_part1_views1357_imgfirst.jsonl")
    ...
    for scene_folder in scene_folders:
        ...
        try:
            ...
            # 如果步骤数量大于10，则跳过
            if len(data['blocks']) > 10:
                print(f"  Skipping scene {scene_name}: {len(data['blocks'])} blocks (> 10)")
                continue
